...
const createScene = async function() {
   ...
   const vase = BABYLON.MeshBuilder.CreateBox("vase", { width: 0.5, height: 1, depth: 0.5 }, scene);
    const vaseMat = new BABYLON.StandardMaterial("vaseMat");
    vaseMat.diffuseColor = new BABYLON.Color3(1, 0, 1);
    vase.material = vaseMat;

    // STEP 2a: The local coordinates origin of the cube car is the reference point for scaling and positioning it within the scene - move it up 0.5 so that it sits on the ground
    vase.position = new BABYLON.Vector3(1, 0.5, -0.75);
    // STEP 2b: Move it out of the way so that the 0,0,0 point in the scene is visible for the next mesh we will import
    ...
    // STEP 4a: Drop the wheel into the scene using the ImportMeshAsync method
    const vaseMesh2 = BABYLON.SceneLoader.ImportMeshAsync("", "./meshes/", "vase.glb").then((result) => {
        
        const vaseMesh = result.meshes[0];
        
        const vaseBounds = result.meshes[1];
        vaseBounds.showBoundingBox = true;
       
        vaseMesh.scaling = new BABYLON.Vector3(30, 30, 30);
       
        vaseMesh.parent = vase;
       
        vaseMesh.position = new BABYLON.Vector3(-4.1, -0.5, 2.5);
    }).catch((error) => {
    ...
    // 2
        const vaseMesh3 = BABYLON.SceneLoader.ImportMeshAsync("", "./meshes/", "vase.glb").then((result) => {
        ...
        vaseMesh.position = new BABYLON.Vector3(-0.6, -0.5, 1.8);
    }).catch((error) => {
    ...
    // 3
        const vaseMesh4 = BABYLON.SceneLoader.ImportMeshAsync("", "./meshes/", "vase.glb").then((result) => {
        ...
        vaseMesh.position = new BABYLON.Vector3(-0, -0.5, -2.3);
    }).catch((error) => {
