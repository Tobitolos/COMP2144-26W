...
const createScene = async function() {
    ...
    scene.onPointerDown = async () => {
        if (lastHitTest && marker.isVisible) {
            ...
            // STEP 8d LAB C: Build a random 3D shape to drop on the surface
            const randomShape = buildRandomShape();
            // STEP 8e LAB C: Attach the random shape to the real world!
            anchor.attachedNode = randomShape;
        }    
    }    
    
    
    /* LAB C: Function to create a random 3D shape with random size and colour
    ---------------------------------------------------------------------------------------------------- */
    function buildRandomShape() {
        // Randomly choose shape type: 0=box, 1=sphere, 2=cylinder
        const shapeType = Math.floor(Math.random() * 3);
        let mesh;
        const size = 0.05 + Math.random() * 0.15; // random size 0.05 to 0.2
        ...
        // Lift mesh so it sits on surface
